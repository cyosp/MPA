...
string Operation::executeGetRequest(ptree & root)
{
	...
	vector<mpapo::Operation> operations = getOperations( accountId );
...
string Operation::executePostAddRequest(ptree & root)
{
	...
	vector<mpapo::Operation> accountOpSize = Account::getAccount( accountId ).operations().get().all();
	...
	// Get provider
	mpapo::Provider provider = Provider::getProvider( accountId , providerName );
	...
	// Get category
	mpapo::Category category = Category::getCategory( accountId , categoryName );
	...
	// Get account
	mpapo::Account account = Account::getAccount( accountId );
	// Update account balance
...
{
	return Account::getAccount( accountId ).operations().get().all();
}
