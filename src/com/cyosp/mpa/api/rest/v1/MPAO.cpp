...
namespace mpa_api_rest_v1
{
    ...
    MPAO::MPAO(HttpRequestType httpRequestType, ActionType actionType, const map<string, string>& argvals,
        ...
        if( restrictedAccess )
        {
            ...
            if( tokenIt != argvals.end() )
            {
                ...
                if( it != MPAOFactory::getInstance()->getTokenList().end() )
                {
                    ...
                    badParamsMsg = MPA::getInstance()->getResourceBundle().translate(INVALID_TOKEN,
                            MPA::DEFAULT_LOCALE);
            }
            else
                badParamsMsg = MPA::getInstance()->getResourceBundle().translate(TOKEN_ARGUMENT_NOT_FOUND,
                        MPA::DEFAULT_LOCALE);
        }
        else
            ret = true;
    ...
    bool MPAO::arePostDeleteParametersOk()
    {
        ...
        if( argvals.find(VERSION_PARAM) != argvals.end() )
        {
            version = atoi(argvals.find(VERSION_PARAM)->second);
