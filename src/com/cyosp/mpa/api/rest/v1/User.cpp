...
namespace mpa_api_rest_v1
{
    ...
    string User::URL_STRING_PATH_IDENTIFIER = "users";

    User::User(HttpRequestType httpRequestType, ActionType actionType, const map<string, string>& argvals,
    ...
    bool User::arePostAddParametersOk()
    {
        ...
        if( loginIt != endIt && passwordIt != endIt && passwordConfirmIt != endIt && localeIt != endIt )
        {
            ...
            if( passwordIt->second.compare(passwordConfirmIt->second) == 0 )
            {
                ...
                badParamsMsg = MPA::getInstance()->getResourceBundle().translate(PASSWORDS_DONT_MATCH, locale);
        }
        else
            badParamsMsg = MPA::getInstance()->getResourceBundle().translate(THERE_ARE_MISSING_PARAMETERS, locale);
    ...
    bool User::arePostDeleteParametersOk()
    {
        ...
        if( argvals.find("version") != argvals.end() && argvals.find("token") != argvals.end() )
            ret = true;
    ...
    bool User::arePostUpdateParametersOk()
    {
        ...
        if( argvals.find("name") != argvals.end() && argvals.find("version") != argvals.end()
        ...
        if( !isAdminRegistered || mpa::User::getUser(login).isAdmin )
        {
            ...
            if( !mpa::User::existUser(login) )
            {
                ...
                else
                {
                    ...
                    char stemp[512] = "";
                    snprintf(stemp, 512, ret.c_str(), StrUtil::int2string(MPA::PWD_SECURITY_MIN_SIZE).c_str(),
                            StrUtil::int2string(MPA::PWD_SECURITY_UPPER_CASE_NBR).c_str(),
                            StrUtil::int2string(MPA::PWD_SECURITY_DIGIT_NBR).c_str());
                    ret = stemp;
                }
            }
            else
                ret = MPA::getInstance()->getResourceBundle().translate(USER_ALREADY_EXISTS, locale);
        }
